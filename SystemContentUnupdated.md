One skill sheet per kept control — a short document, same twelve headings every time, so a peer could rerun your checks. 

1. METADATA — Control id(s); ASVS or AISVS; standard + version; P0/P1 (P0 most importand, P1 less importand); scope (web / api / llm_ai); author; date. Lab evidence boundary (what lab can / cannot prove vs production). prerequisite control refs.
2. REQUIREMENT SUMMARY — Faithful clause summary; link to source.
3. PENTEST APPLICABILITY — In-scope vs out-of-scope; no over-claim from lab-only evidence.
4. THREAT / ABUSE SCENARIOS — Scenarios + safety: halt if RoE or safety is at risk.
5. PRECONDITIONS — Accounts, roles, data, environment; repeatability anchors (Phase D); stop conditions.
6. MANUAL TEST PROCEDURE — Numbered steps; pass / fail / inconclusive when evidence is incomplete.
7. TOOLING & ASSISTED TESTING — Hypothesis vs evidence; LLM non-determinism handling if relevant.
8. EVIDENCE SPECIFICATION — Artefacts; redaction; storage; retention if set; sensitivity / handling.
9. TRIAGE & VERDICT RULES — pass | partial | fail | N/A; inconclusive | blocked when appropriate; align with programme CSV / reports.
10. FALSE POSITIVES & NOISE — Typical noise; confirm, downgrade, or discard.
11. CROSS-REFERENCES — WSTG, API Top 10, LLM Top 10, PortSwigger, Cheat Sheets; related controls + dependencies.

If a section does not apply: "N/A — one sentence why." No empty placeholders at DA.
so these are part of the rules let me add some other details be concise its should be bullet points and a high level view of the control except for the following header: scope generate a more detailed explanation, pentest applicability give example more supstance and 1 to 3 examples depending on the control conplexity make them actionable, be as accurate as you can in what the control means, below ill add the control  but that does not mean you dont have to use the ASVS file or the internet as a reference. keep it under 100 lines. make it more of a summary and to the point. for the menual test procedure only note that down if human interaction is needed if it can be automated just note down "can be automated". let me give an small guide for that if the control needs data collection but a human need to look over that data note that down. if it cant be automated note down a manual test procedure and when you do that make in consice, and bullet points not a step by step guide. 



the info below is an template so just use that exactly and fill it in like a question form only keep the auther and date the same
...
- **Scope:** llm_ai, data pipelines, training environments  
  *Scope refers to the full lifecycle of training data, including sourcing, ingestion, preprocessing, and model retraining pipelines, primarily in AI/ML training contexts.*  
...
- **Sensitivity:**  
  - there Classify artifacts as confidential, secure per internal policies.
